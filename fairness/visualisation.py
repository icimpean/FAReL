...
class FairnessVisualisation(object):
    ...
    def _print_probabilities_group(self, name, f_0, prob_0, f_1, prob_1, cm_formula, threshold, exact, approx, y_labels=None):
        print(f"{name}: {f_0} == {f_1}")
        print(cm_formula, "should be equal for groups")
        ...
        def _print(p_0, p_1, l):
            ...
            print(f"\tFound{l}: {p_0} {'!' if p_0 != p_1 else '='}= {p_1}")
        ...
        print(f"\t{prefix} under exact constraint")

        # Approximate fairness
        if threshold is not None:
            print(f"\tWith threshold: | {f_0} - {f_1} | ≤ threshold")

            def _thr(p_0, p_1, l):
                l = "" if l is None else f"For {l}: "
                w_thresh = abs(p_0 - p_1)
                sign = "≤" if w_thresh <= threshold else "≰"
                print(f"\t                  {l}{w_thresh} {sign} {threshold}")
            ...
            print(f"\t                  {prefix} under approximate constraint")
    ...
    def _print_probabilities_individual(self, name, formula, description, threshold, exact, approx, diff,
        ...
        def _print(ui, up):
            if (len(ui) == 0) and (len(up) == 0):
                ...
                text_u = "individuals"
                lu = ui
            print(f"\tFound {len_u} unsatisfied {text_u}{':    ' + str(lu) if lu is not None else ''}")
    ...
    def print_individual_notion(self, notion: IndividualNotion, threshold, exact, approx, diff, u_ind, u_pairs):
        ...
        self._print_probabilities_individual(notion_name, text_formula, text_description, threshold, exact, approx,
                                             diff, u_ind, u_pairs)
    ...
    def get_fairness_radar_plot(self, fairness_framework: FairnessFramework, requested_notions, threshold,
        ...
        self.bar_plot2(labels, (differences, exacts, approxs), -threshold)
    ...
    def bar_plot2(self, labels, data, threshold):
        ...
        for i, (label, diff, e, a) in enumerate(zip(labels, differences, exact, approx)):
            col = col_exact if e else (col_approx if a else col_unfair)
            plt.barh(i, diff, color=col)
            plt.text(threshold - 0.005, i + 0.0, label, rotation=0,
        ...
        plt.vlines(threshold, -2, len(labels) + 1, color="black")

        plt.ylim(-0.5, len(labels) - 0.5)
        # plt.xlim(0, max(-1000, min(np.nan_to_num(differences))) - 0.01)
        # TODO: treatment equality isn't bounded to [0, 1] interval => switch diff FN/FP to FP/FN?
        plt.xlim(0, max(-1.0, min(np.nan_to_num(differences))) - 0.01)
        # TODO
        # plt.xlim(0, -0.16 - 0.01)
        plt.ylabel("Fairness Notions", fontsize=14)
        plt.xlabel("Unfairness", fontsize=14)
