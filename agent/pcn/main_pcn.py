...
class HistoryEnv(gym.Wrapper):
    ...
    def step(self, ac):
        state, r, d, _, i = self.env.step(ac)
        # shift history
        self._state = np.roll(self._state, -1, axis=0)
        # add state to history
        self._state[-1] = state
        return np.concatenate(self._state, axis=0), r, d, _, i
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='densesmall', type=str, help='conv1d(big|small), dense(big|small)')
