...
from fairness.visualisation import FairnessVisualisation


def run_scenario(agent: Agent, env_init, env_train, env_eval, steps_init, steps_train, steps_eval,
    ...
    if steps_init > 0:
        ...
        current_t = _run(agent, current_t, steps_init, env_init, fairness_framework[0], fairness_framework_windowed[0])

    if steps_train > 0:
        print("TRAINING...")
        # agent.training = True
        agent.set_training(train=True)
        current_t = _run(agent, current_t, steps_train, env_train, fairness_framework[1], fairness_framework_windowed[1])

    if steps_eval > 0:
        print("Evaluating...")
        # agent.training = False
        agent.set_training(train=False)
        current_t = _run(agent, current_t, steps_eval, env_eval, fairness_framework[2], fairness_framework_windowed[2])
...
def _run(agent, current_t, steps, env, fairness_framework: FairnessFramework,
    ...
    for t in range(current_t, current_t + steps):
        ...
        print(f"t={t:5d} | {ep_info}action={env.actions[action].name} ({action}), reward={reward:5f} (true_action: {info['true_action']})"
              f"{tab if notion_info != '' else ''}{notion_info}")
        state = next_state
    ...
    vis = FairnessVisualisation(env.actions)
        ...
        if framework:
            ...
            if framework.visualise_notions:
                ...
                for notion in framework.individual_notions:
                    ...
                    vis.print_individual_notion(notion, framework.threshold, exact, approx, diff, u_ind, u_pairs)

                    if framework.visualise_reward:
                        vis.fairness_notion_reward(framework, notion, framework.threshold, sensitive_attribute=None)

            # For each sensitive attribute
            for sensitive_attribute in framework.sensitive_attributes:
                sensitive_feature = sensitive_attribute.features
                ...
                vis.confusion_matrix(cm, sensitive_feature=None, sensitive_value=sensitive_value, other_value=other_value,
                ...
                vis.confusion_matrix(cm, sensitive_feature=sensitive_feature, sensitive_value=sensitive_value,
                                     other_value=other_value, is_sensitive_value=True, print_cm=True, plot_cm=framework.visualise_cm)
                # Others
                ...
                vis.confusion_matrix(cm, sensitive_feature=sensitive_feature, sensitive_value=sensitive_value,
                                     other_value=other_value, is_sensitive_value=False, print_cm=True, plot_cm=framework.visualise_cm)
                ...
                if framework.visualise_notions:
                    ...
                    for notion in framework.group_notions:
                        ...
                        vis.print_group_notion(notion, sensitive_feature, sensitive_value, other_value,
                                               framework.threshold, exact, approx,
                                               prob_sensitive, prob_other)

                        if framework.visualise_reward:
                            vis.fairness_notion_reward(framework, notion, framework.threshold, sensitive_attribute)

            if framework.visualise:
                vis.get_fairness_radar_plot(framework, framework.all_notions, framework.threshold,
